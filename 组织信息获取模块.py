...
headtext = ["排名", "落 网 时 间", "职务", "头   衔", "竞赛", "联赛", "星", "捐兵", "收兵", "额外奖励", "组织内部代号"]
tree = ttk.Treeview(root, height=18, show="headings", style="mystyle.Treeview", columns=columns)  # 调用外部样式
...
tree.column("three", width=95)
tree.column("four", width=55)
tree.column("five", width=37)
...
while dd:
    ...
    # print(dd.rstrip())  # 删除末尾的空行
    # dd.rstrip()
    if dd == '':  # read()到达文件末尾时返回一个空字符串，而将这个空字符串显示出来就是一个空行
        break  # 由于未新建变量储存文本值，strip()和rstrip()无效，使用判断语句制止创建GUI空行
    ...
    if xulie >= 10:  # Sort()排序按照ASCII码，比较位数上的大小，此番修改了与需求所需的偏差
        cyname.insert(0, xulie)
    elif xulie < 10:
        cyname.insert(0, buding_xulie[xulie])
    xulie += 1  # 判断块'前 后',从'零'开始产生名次顺序(else依赖 注意修改列表或插入语句)
    tree.insert("", xulie, values=cyname)#,tags = ('even',))
    #print(type(xulie), xulie)  测试排名序列 点击排序效果
    # print(dd.split(", "))
...
#tree.pack(fill='both', expand=1) # 仅50当前在线成员列表名单提交

#==============================================================================离线成员
memberList = open('afieldList.txt', mode='rt')
member = 1
nameRanking = 51
while member:
    member = memberList.readline()
    if member == '':
        break
    offlinename = member.split(", ")
    if nameRanking >= 51:
        offlinename.insert(0, nameRanking)
    nameRanking += 1
    tree.insert("", nameRanking, values=offlinename)

id1 = tree.insert("", nameRanking)
print('1')
tree.insert(id1, "end", values=("部落对战管理员", "戏子", "二  2018.9"))
print('2')
tree.insert(id1, "end", values=['一  2018.09.00', '副首', '部落专门收兵的', '08-4000', '参战', '0', '10000', '10000', '捐兵破万', '魔域源'])
print('3')
...
def tipscopy(event):
    ...
    tipwindows.geometry('545x45')  # 大小和列表长度相关
    tipwindows.resizable(False, False)
    tipwindows.title(tiptle)
    e1=tk.Entry(tipwindows, textvariable=content, state="readonly",)
    e1.pack(padx=10, pady=10, fill='both', expand=1)
...
tree.bind('<ButtonRelease-1>', tipscopy)  # 绑定右键双击离开事件===========
